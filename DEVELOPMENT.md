...
- **Trigger name**: `trg_purchases_{descriptive_name}`
- **Function name**: `trg_purchases_{descriptive_name}_fn`

Where `purchases` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_purchases_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_purchases_audit_log"
AFTER INSERT OR UPDATE ON "module_purchases_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_purchases_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_purchases_*` triggers when the plugin
